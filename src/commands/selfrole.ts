...
export const selfroleCommand: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === 'add') {
      ...
      if (role.id === guild.id) {
        await interaction.reply({ content: 'You cannot add @everyone.', flags: MessageFlags.Ephemeral });
        return;
      }
    ...
    if (sub === 'list') {
      ...
      if (data.roles.length === 0) {
        await interaction.reply({ content: 'No self-roles configured.', flags: MessageFlags.Ephemeral });
        return;
      }
    ...
    if (sub === 'panel') {
      if (!interaction.channel || !interaction.channel.isTextBased() || interaction.channel.isDMBased()) {
        await interaction.reply({ content: 'The panel can only be posted in a text channel.', flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      await interaction.reply({ content: 'Self-role panel posted.', flags: MessageFlags.Ephemeral });
    }
  },
};
